...
from smf2db.api.api_75 import format_75df
...
def upload_75db(db_engines: dict, db_session: Session, jsonfiles: str, partitions_scheme: str,
    ...
    for jsonfile in jsonfiles:
        ...
        with open(jsonfile) as f:
            ...
            if dfs_dict['pro'].empty:
                ...
                csc = get_csc_in_db(dfs_dict['pro']['smf75xnm'].unique()[0], dfs_dict['pro']['smf75snm'].unique()[0],
                                    pd.to_numeric(dfs_dict['pro']['smf75ptn'].unique()[0], downcast="integer"),
                                    db_session)
                if csc is None:
                    click.echo(
            ...
            for table in insert_dict.keys():
                if dfs_dict[table].shape[0] > 0:
                    insert_dict[table] = df_insert_by_partitions(db_driver, db_engines, db_session,
                                                                 ...
                                                                 tblnames[table], tbls[table],
                                                                'smf75', int_dtypedict[table], '75',
                                                                 partitions_scheme, True)
