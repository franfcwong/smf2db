...
from .smf74_base import ReprMixin, Base74Hr, Smf74pro, Smf74dctl, Smf74dev, Smf74cf, Smf74lcf, Smf74gsrg, Smf74proc, \
...
class Smf74ProHr(ReprMixin, Base74Hr, Smf74pro):
    """The Smf74ProHr class stores the hourly Smf74Pro record in the smf74_pro_hr table."""

    __tablename__ = "smf74_pro_hr"
    ...
    __table_args__ = (
        sa.PrimaryKeyConstraint(datetime, smf_type, csc, smf74sid),
    )

    smf74_xctl_hr: so.Mapped['Smf74XctlHr'] = so.relationship(back_populates='smf74_pro_hr', viewonly=True)
    smf74_omvs_hr: so.Mapped['Smf74OmvsHr'] = so.relationship(back_populates='smf74_pro_hr', viewonly=True)
    smf74_hfs_hr: so.Mapped['Smf74HfsHr'] = so.relationship(back_populates='smf74_pro_hr', viewonly=True)
    smf74_fcd_hr: so.Mapped['Smf74FcdHr'] = so.relationship(back_populates='smf74_pro_hrs', viewonly=True,
                                                            foreign_keys=[datetime, smf_type],
                                                            primaryjoin='and_(Smf74ProHr.datetime==Smf74FcdHr.datetime, Smf74ProHr.smf_type==Smf74FcdHr.smf_type)', )
    smf74_cntl_hr: so.Mapped['Smf74CntlHr'] = so.relationship(back_populates='smf74_pro_hrs', viewonly=True,
                                                              foreign_keys=[datetime, smf_type],
                                                              primaryjoin='and_(Smf74ProHr.datetime==Smf74CntlHr.datetime, Smf74ProHr.smf_type==Smf74CntlHr.smf_type)', )
    smf74_cachsys_hr: so.Mapped['Smf74CachsysHr'] = so.relationship(back_populates='smf74_pro_hrs', viewonly=True,
                                                                    foreign_keys=[datetime, smf_type],
                                                                    primaryjoin='and_(Smf74ProHr.datetime==Smf74CachsysHr.datetime, Smf74ProHr.smf_type==Smf74CachsysHr.smf_type)', )
    smf74_pcie_hr: so.Mapped['Smf74PcieHr'] = so.relationship(back_populates='smf74_pro_hrs', viewonly=True,
                                                              foreign_keys=[datetime, smf_type],
                                                              primaryjoin='and_(Smf74ProHr.datetime==Smf74PcieHr.datetime, Smf74ProHr.smf_type==Smf74PcieHr.smf_type)', )
    smf74_scm_hr: so.Mapped['Smf74ScmHr'] = so.relationship(back_populates='smf74_pro_hrs', viewonly=True)
    smf74_eadm_hr: so.Mapped['Smf74EadmHr'] = so.relationship(back_populates='smf74_pro_hrs', viewonly=True)
    smf74_dctl_hrs: so.Mapped[List['Smf74DctlHr']] = so.relationship(back_populates='smf74_pro_hr', viewonly=True)
    smf74_lcf_hrs: so.Mapped[List['Smf74LcfHr']] = so.relationship(back_populates='smf74_pro_hr', viewonly=True)


class Smf74LcfHr(ReprMixin, Base74Hr, Smf74lcf):
    """The Smf74LcfHr class stores the hourly Smf74Lcf record in the smf74_lcf_hr table."""

    __tablename__ = "smf74_lcf_hr"
    ...
    __table_args__ = (
        sa.PrimaryKeyConstraint(smf74xnm, datetime, r744fnam, smf74sid),
        sa.ForeignKeyConstraint(
            ['datetime', 'smf_type', 'csc', 'smf74sid'],
            ['smf74_pro_hr.datetime', 'smf74_pro_hr.smf_type', 'smf74_pro_hr.csc', 'smf74_pro_hr.smf74sid']),
        sa.ForeignKeyConstraint(
            ['smf74xnm', 'datetime', 'r744fnam'],
            ['smf74_cf_hr.smf74xnm', 'smf74_cf_hr.datetime', 'smf74_cf_hr.r744fnam']),
    )

    smf74_cf_hr: so.Mapped['Smf74CfHr'] = so.relationship(back_populates='smf74_lcf_hrs', viewonly=True)
    smf74_pro_hr: so.Mapped['Smf74ProHr'] = so.relationship(back_populates='smf74_lcf_hrs', viewonly=True)
    smf74_sreq_hrs: so.Mapped[List['Smf74SreqHr']] = so.relationship(back_populates='smf74_lcf_hr', viewonly=True)


class Smf74ArryHr(ReprMixin, Base74Hr, Smf74arry):
    """The Smf74ArryHr class stores the hourly Smf74Arry record in the smf74_arry_hr table."""

    __tablename__ = "smf74_arry_hr"
    ...
    __table_args__ = (
        sa.PrimaryKeyConstraint(datetime, r748cser, r748arid, r748aaid),
        sa.ForeignKeyConstraint(
            ['datetime', 'r748cser', 'r748arid'],
            ['smf74_rank_hr.datetime', 'smf74_rank_hr.r748cser', 'smf74_rank_hr.r748rrid']),
    )

    smf74_rank_hr: so.Mapped['Smf74RankHr'] = so.relationship(back_populates='smf74_arry_hrs', viewonly=True)


class Smf74CdevHr(ReprMixin, Base74Hr, Smf74cdev, Smf741dev):
    """The Smf74CdevHr class stores the hourly Smf74Cdev record in the smf74_cdev_hr table."""

    __tablename__ = "smf74_cdev_hr"
    ...
    __table_args__ = (
        sa.PrimaryKeyConstraint(datetime, r745dsid, r745devn),
        sa.ForeignKeyConstraint(
            ['datetime', 'r745dsid'],
            ['smf74_cachsys_hr.datetime', 'smf74_cachsys_hr.r745ssid']),
    )

    smf74_raid_hr: so.Mapped['Smf74RaidHr'] = so.relationship(back_populates='smf74_cdev_hr', viewonly=True)
    smf74_xpool_hr: so.Mapped['Smf74XpoolHr'] = so.relationship(back_populates='smf74_cdev_hr', viewonly=True)
    smf74_cachsys_hr: so.Mapped['Smf74CachsysHr'] = so.relationship(back_populates='smf74_cdev_hrs', viewonly=True)


class Smf74RankHr(ReprMixin, Base74Hr, Smf74rank, Smf74arry):
    """The Smf74RankHr class stores the hourly Smf74Rank record in the smf74_rank_hr table."""

    __tablename__ = "smf74_rank_hr"
    ...
    __table_args__ = (
        sa.PrimaryKeyConstraint(datetime, r748cser, r748rrid),
        sa.ForeignKeyConstraint(
            ['datetime', 'r748cser'],
            ['smf74_cntl_hr.datetime', 'smf74_cntl_hr.r748cser']),
        sa.ForeignKeyConstraint(
            ['datetime', 'r748cser', 'r748rpnm'],
            ['smf74_extp_hr.datetime', 'smf74_extp_hr.r748cser', 'smf74_extp_hr.r748xpid']),
    )

    smf74_cntl_hr: so.Mapped['Smf74CntlHr'] = so.relationship(back_populates='smf74_rank_hrs', viewonly=True)
    smf74_extp_hr: so.Mapped['Smf74ExtpHr'] = so.relationship(back_populates='smf74_rank_hrs', viewonly=True)
    smf74_arry_hrs: so.Mapped[List['Smf74ArryHr']] = so.relationship(back_populates='smf74_rank_hr', viewonly=True)


class Smf74XpoolHr(ReprMixin, Base74Hr, Smf74xpool, Smf741dev):
    """The Smf74XpoolHr class stores the hourly Smf74Xpool record in the smf74_xpool_hr table."""

    __tablename__ = "smf74_xpool_hr"
    ...
    __table_args__ = (
        sa.PrimaryKeyConstraint(datetime, r745ssid, r7451dvn),
        sa.ForeignKeyConstraint(
            ['datetime', 'r745ssid', 'r7451dvn'],
            ['smf74_cdev_hr.datetime', 'smf74_cdev_hr.r745dsid', 'smf74_cdev_hr.r745devn']),
    )

    smf74_cdev_hr: so.Mapped['Smf74CdevHr'] = so.relationship(back_populates='smf74_xpool_hr', viewonly=True)


class Smf74AdupHr(ReprMixin, Base74Hr, Smf74adup):
    """The Smf74AdupHr class stores the hourly Smf74Adup record in the smf74_adup_hr table."""

    __tablename__ = "smf74_adup_hr"
    csc: so.Mapped[Optional[str]] = so.mapped_column(sa.String(8),
                                                     doc="the identifier or serial number of Central Processor Complexes (CPC).")
    date: so.Mapped[Optional[dt.date]] = so.mapped_column(sa.Date, doc="the date of the record.")
    last_update_time: so.Mapped[Optional[dt.datetime]] = so.mapped_column(sa.DateTime,
    ...
    __table_args__ = (
        sa.PrimaryKeyConstraint(smf74xnm, datetime, r744fnam, r744snam, smf74sid),
        sa.ForeignKeyConstraint(
            ['smf74xnm', 'datetime', 'r744fnam', 'r744snam', 'smf74sid'],
            ['smf74_sreq_hr.smf74xnm', 'smf74_sreq_hr.datetime', 'smf74_sreq_hr.r744fnam', 'smf74_sreq_hr.r744snam',
             'smf74_sreq_hr.smf74sid']),
        sa.ForeignKeyConstraint(
            ['smf74xnm', 'datetime', 'r744fnam', 'r744snam'],
            ['smf74_str_hr.smf74xnm', 'smf74_str_hr.datetime', 'smf74_str_hr.r744fnam', 'smf74_str_hr.r744qstr']),
    )

    smf74_sreq_hr: so.Mapped['Smf74SreqHr'] = so.relationship(back_populates='smf74_adup_hrs', viewonly=True)
    smf74_str_hr: so.Mapped['Smf74StrHr'] = so.relationship(back_populates='smf74_adup_hrs', viewonly=True)


class Smf74CachHr(ReprMixin, Base74Hr, Smf74cach):
    """The Smf74CachHr class stores the hourly Smf74Cach record in the smf74_cach_hr table."""

    __tablename__ = "smf74_cach_hr"
    csc: so.Mapped[Optional[str]] = so.mapped_column(sa.String(8),
                                                     doc="the identifier or serial number of Central Processor Complexes (CPC).")
    date: so.Mapped[Optional[dt.date]] = so.mapped_column(sa.Date, doc="the date of the record.")
    last_update_time: so.Mapped[Optional[dt.datetime]] = so.mapped_column(sa.DateTime,
    ...
    smf74_sreq_hr: so.Mapped['Smf74SreqHr'] = so.relationship(back_populates='smf74_cach_hrs', viewonly=True)


class Smf74CachsysHr(ReprMixin, Base74Hr, Smf74cachsys, Smf74cdata, Smf74cdev, Smf741dev):
    """The Smf74CachsysHr class stores the hourly Smf74Cachsys record in the smf74_cachsys_hr table."""

    __tablename__ = "smf74_cachsys_hr"
    ccmt_seqn: so.Mapped[Optional[str]] = so.mapped_column(sa.String(10),
                                                           doc="the sequence number of the subsystem control unit.")
    smf_type: so.Mapped[Optional[str]] = so.mapped_column(sa.String(5), doc="the SMF type and subtype.")
    r7452pro: so.Mapped[Optional[float]] = so.mapped_column(sa.Float, doc="Physical storage read operations.")
    ...
    __table_args__ = (
        sa.PrimaryKeyConstraint(datetime, r745ssid),
    )

    smf74_pro_hrs: so.Mapped[List['Smf74ProHr']] = so.relationship(back_populates='smf74_cachsys_hr', viewonly=True,
                                                                   foreign_keys='Smf74ProHr.datetime, Smf74ProHr.smf_type',
                                                                   primaryjoin='and_(Smf74CachsysHr.datetime==Smf74ProHr.datetime, Smf74CachsysHr.smf_type==Smf74ProHr.smf_type)', )
    smf74_cdev_hrs: so.Mapped[List['Smf74CdevHr']] = so.relationship(back_populates='smf74_cachsys_hr', viewonly=True)
    smf74_rrank_hrs: so.Mapped[List['Smf74RrankHr']] = so.relationship(back_populates='smf74_cachsys_hr', viewonly=True)


class Smf74CfrfHr(ReprMixin, Base74Hr, Smf74cfrf):
    """The Smf74CfrfHr class stores the hourly Smf74Cfrf record in the smf74_cfrf_hr table."""

    __tablename__ = "smf74_cfrf_hr"
    ...
    __table_args__ = (
        sa.PrimaryKeyConstraint(smf74xnm, datetime, r744fnam, r744rnam, smf74sid),
        sa.ForeignKeyConstraint(
            ['smf74xnm', 'datetime', 'r744fnam'],
            ['smf74_cf_hr.smf74xnm', 'smf74_cf_hr.datetime', 'smf74_cf_hr.r744fnam']),
    )

    smf74_cf_hr: so.Mapped['Smf74CfHr'] = so.relationship(back_populates='smf74_cfrf_hrs', viewonly=True)
    smf74_dupchpa_hrs: so.Mapped[List['Smf74DupchpaHr']] = so.relationship(back_populates='smf74_cfrf_hr',
                                                                           viewonly=True)


class Smf74DupchpaHr(ReprMixin, Base74Hr, Smf74chpa):
    """The Smf74DupchpaHr class stores the hourly Smf74Dupchpa record in the smf74_dupchpa_hr table."""

    __tablename__ = "smf74_dupchpa_hr"
    csc: so.Mapped[Optional[str]] = so.mapped_column(sa.String(8),
                                                     doc="the identifier or serial number of Central Processor Complexes (CPC).")
    date: so.Mapped[Optional[dt.date]] = so.mapped_column(sa.Date, doc="the date of the record.")
    last_update_time: so.Mapped[Optional[dt.datetime]] = so.mapped_column(sa.DateTime,
    ...
    __table_args__ = (
        sa.PrimaryKeyConstraint(smf74xnm, datetime, r744fnam, r744rnam, r744hcpi, r744hsnd, smf74sid),
        sa.ForeignKeyConstraint(
            ['smf74xnm', 'datetime', 'r744fnam', 'r744rnam', 'smf74sid'],
            ['smf74_cfrf_hr.smf74xnm', 'smf74_cfrf_hr.datetime', 'smf74_cfrf_hr.r744fnam', 'smf74_cfrf_hr.r744rnam',
             'smf74_cfrf_hr.smf74sid']),
    )

    smf74_cfrf_hr: so.Mapped['Smf74CfrfHr'] = so.relationship(back_populates='smf74_dupchpa_hrs', viewonly=True)


class Smf74SubchpaHr(ReprMixin, Base74Hr, Smf74chpa):
    """The Smf74SubchpaHr class stores the hourly Smf74Subchpa record in the smf74_subchpa_hr table."""

    __tablename__ = "smf74_subchpa_hr"
    csc: so.Mapped[Optional[str]] = so.mapped_column(sa.String(8),
                                                     doc="the identifier or serial number of Central Processor Complexes (CPC).")
    date: so.Mapped[Optional[dt.date]] = so.mapped_column(sa.Date, doc="the date of the record.")
    last_update_time: so.Mapped[Optional[dt.datetime]] = so.mapped_column(sa.DateTime,
    ...
    __table_args__ = (
        sa.PrimaryKeyConstraint(smf74xnm, datetime, r744fnam, smf74sid, r744hcpi, r744hsnd),
        sa.ForeignKeyConstraint(
            ['smf74xnm', 'datetime', 'r744fnam'],
            ['smf74_cf_hr.smf74xnm', 'smf74_cf_hr.datetime', 'smf74_cf_hr.r744fnam']),
    )

    smf74_cf_hr: so.Mapped['Smf74CfHr'] = so.relationship(back_populates='smf74_subchpa_hrs', viewonly=True)


class Smf74CntlHr(ReprMixin, Base74Hr, Smf74cntl):
    """The Smf74CntlHr class stores the hourly Smf74Cntl record in the smf74_cntl_hr table."""

    __tablename__ = "smf74_cntl_hr"
    r748cser: so.Mapped[str] = so.mapped_column(sa.String(10), doc="Primary control unit serial number.")
    smf_type: so.Mapped[Optional[str]] = so.mapped_column(sa.String(5), doc="the SMF type and subtype.")
    date: so.Mapped[Optional[dt.date]] = so.mapped_column(sa.Date, doc="the date of the record.")
    last_update_time: so.Mapped[Optional[dt.datetime]] = so.mapped_column(sa.DateTime,
    ...
    __table_args__ = (
        sa.PrimaryKeyConstraint(datetime, r748cser),
    )

    smf74_pro_hrs: so.Mapped[List['Smf74ProHr']] = so.relationship(back_populates='smf74_cntl_hr', viewonly=True,
                                                                   foreign_keys='Smf74ProHr.datetime, Smf74ProHr.smf_type',
                                                                   primaryjoin='and_(Smf74CntlHr.datetime==Smf74ProHr.datetime, Smf74CntlHr.smf_type==Smf74ProHr.smf_type)', )
    smf74_lss_hrs: so.Mapped[List['Smf74LssHr']] = so.relationship(back_populates='smf74_cntl_hr', viewonly=True)
    smf74_extp_hrs: so.Mapped[List['Smf74ExtpHr']] = so.relationship(back_populates='smf74_cntl_hr', viewonly=True)
    smf74_rank_hrs: so.Mapped[List['Smf74RankHr']] = so.relationship(back_populates='smf74_cntl_hr', viewonly=True)
    smf74_siol_hrs: so.Mapped[List['Smf74SiolHr']] = so.relationship(back_populates='smf74_cntl_hr', viewonly=True)


class Smf74ConnectorHr(ReprMixin, Base74Hr, Smf74connector):
    """The Smf74ConnectorHr class stores the hourly Smf74Connector record in the smf74_connector_hr table."""

    __tablename__ = "smf74_connector_hr"
    ...
    __table_args__ = (
        sa.PrimaryKeyConstraint(datetime, r747sdev, r747cadr, r747cnum, r747ccu),
        sa.ForeignKeyConstraint(
            ['datetime', 'r747sdev', 'r747cadr', 'r747cnum'],
            ['smf74_port_hr.datetime', 'smf74_port_hr.r747sdev', 'smf74_port_hr.r747padr', 'smf74_port_hr.r747pnum']),
    )

    smf74_port_hr: so.Mapped['Smf74PortHr'] = so.relationship(back_populates='smf74_connector_hrs', viewonly=True)


class Smf74DctlHr(ReprMixin, Base74Hr, Smf74dctl):
    """The Smf74DctlHr class stores the hourly Smf74Dctl record in the smf74_dctl_hr table."""

    __tablename__ = "smf74_dctl_hr"
    ...
    __table_args__ = (
        sa.PrimaryKeyConstraint(csc, smf74sid, datetime, smf74sub),
        sa.ForeignKeyConstraint(
            ['datetime', 'smf_type', 'csc', 'smf74sid'],
            ['smf74_pro_hr.datetime', 'smf74_pro_hr.smf_type', 'smf74_pro_hr.csc', 'smf74_pro_hr.smf74sid']),
    )

    smf74_pro_hr: so.Mapped['Smf74ProHr'] = so.relationship(back_populates='smf74_dctl_hrs', viewonly=True)
    smf74_dev_hrs: so.Mapped[List['Smf74DevHr']] = so.relationship(back_populates='smf74_dctl_hr', viewonly=True)


class Smf74DevHr(ReprMixin, Base74Hr, Smf74dev):
    """The Smf74DevHr class stores the hourly Smf74Dev record in the smf74_dev_hr table."""

    __tablename__ = "smf74_dev_hr"
    ...
    __table_args__ = (
        sa.PrimaryKeyConstraint(csc, smf74sid, datetime, smf74sub, smf74lcu, smf74scs, smf74num),
        sa.ForeignKeyConstraint(
            ['csc', 'smf74sid', 'datetime', 'smf74sub'],
            ['smf74_dctl_hr.csc', 'smf74_dctl_hr.smf74sid', 'smf74_dctl_hr.datetime', 'smf74_dctl_hr.smf74sub']),
    )

    smf74_dctl_hr: so.Mapped['Smf74DctlHr'] = so.relationship(back_populates='smf74_dev_hrs', viewonly=True)


class Smf74EadmHr(ReprMixin, Base74Hr, Smf74eadm):
    """The Smf74EadmHr class stores the hourly Smf74Eadm record in the smf74_eadm_hr table."""

    __tablename__ = "smf74_eadm_hr"
    smf_type: so.Mapped[Optional[str]] = so.mapped_column(sa.String(5), doc="the SMF type and subtype.")
    date: so.Mapped[Optional[dt.date]] = so.mapped_column(sa.Date, doc="the date of the record.")
    last_update_time: so.Mapped[Optional[dt.datetime]] = so.mapped_column(sa.DateTime,
                                                                          doc="last update time of this record.")
    ...
    __table_args__ = (
        sa.PrimaryKeyConstraint(csc, smf74sid, datetime),
        sa.ForeignKeyConstraint(
            ['datetime', 'smf_type', 'csc', 'smf74sid'],
            ['smf74_pro_hr.datetime', 'smf74_pro_hr.smf_type', 'smf74_pro_hr.csc', 'smf74_pro_hr.smf74sid']),
    )

    smf74_pro_hrs: so.Mapped[List['Smf74ProHr']] = so.relationship(back_populates='smf74_eadm_hr', viewonly=True)


class Smf74ExtpHr(ReprMixin, Base74Hr, Smf74extp, Smf74rank, Smf74arry):
    """The Smf74ExtpHr class stores the hourly Smf74Extp record in the smf74_extp_hr table."""

    __tablename__ = "smf74_extp_hr"
    ...
    __table_args__ = (
        sa.PrimaryKeyConstraint(datetime, r748cser, r748xpid),
        sa.ForeignKeyConstraint(
            ['datetime', 'r748cser'],
            ['smf74_cntl_hr.datetime', 'smf74_cntl_hr.r748cser']),
    )

    smf74_cntl_hr: so.Mapped['Smf74CntlHr'] = so.relationship(back_populates='smf74_extp_hrs', viewonly=True)
    smf74_rank_hrs: so.Mapped[List['Smf74RankHr']] = so.relationship(back_populates='smf74_extp_hr', viewonly=True)


class Smf74FcdHr(ReprMixin, Base74Hr, Smf74fcd, Smf74pstat):
    """The Smf74FcdHr class stores the hourly Smf74Fcd record in the smf74_fcd_hr table."""

    __tablename__ = "smf74_fcd_hr"
    smf_type: so.Mapped[Optional[str]] = so.mapped_column(sa.String(5), doc="the SMF type and subtype.")
    last_update_time: so.Mapped[Optional[dt.datetime]] = so.mapped_column(sa.DateTime,
                                                                          doc="last update time of this record.")
    date: so.Mapped[Optional[dt.date]] = so.mapped_column(sa.Date, doc="the date of the record.")
    ...
    smf74_pro_hrs: so.Mapped[List['Smf74ProHr']] = so.relationship(back_populates='smf74_fcd_hr', viewonly=True,
                                                                   foreign_keys='Smf74ProHr.datetime, Smf74ProHr.smf_type',
                                                                   primaryjoin='and_(Smf74FcdHr.datetime==Smf74ProHr.datetime, Smf74FcdHr.smf_type==Smf74ProHr.smf_type)', )
    smf74_switch_hrs: so.Mapped[List['Smf74SwitchHr']] = so.relationship(back_populates='smf74_fcd_hr', viewonly=True)


class Smf74FsysHr(ReprMixin, Base74Hr, Smf74fsys):
    """The Smf74FsysHr class stores the hourly Smf74Fsys record in the smf74_fsys_hr table."""

    __tablename__ = "smf74_fsys_hr"
    ...
    __table_args__ = (
        sa.PrimaryKeyConstraint(csc, smf74sid, datetime, r746fsnm),
        sa.ForeignKeyConstraint(
            ['csc', 'smf74sid', 'datetime'],
            ['smf74_hfs_hr.csc', 'smf74_hfs_hr.smf74sid', 'smf74_hfs_hr.datetime']),
    )

    smf74_hfs_hr: so.Mapped['Smf74HfsHr'] = so.relationship(back_populates='smf74_fsys_hrs', viewonly=True)


class Smf74GbufHr(ReprMixin, Base74Hr, Smf74gbuf):
    """The Smf74GbufHr class stores the hourly Smf74Gbuf record in the smf74_gbuf_hr table."""

    __tablename__ = "smf74_gbuf_hr"
    date: so.Mapped[Optional[dt.date]] = so.mapped_column(sa.Date, doc="the date of the record.")
    last_update_time: so.Mapped[Optional[dt.datetime]] = so.mapped_column(sa.DateTime,
                                                                          doc="last update time of this record.")
    ...
    __table_args__ = (
        sa.PrimaryKeyConstraint(csc, smf74sid, datetime, pool_number),
        sa.ForeignKeyConstraint(
            ['csc', 'smf74sid', 'datetime'],
            ['smf74_hfs_hr.csc', 'smf74_hfs_hr.smf74sid', 'smf74_hfs_hr.datetime']),
    )

    smf74_hfs_hr: so.Mapped['Smf74HfsHr'] = so.relationship(back_populates='smf74_gbuf_hrs', viewonly=True)


class Smf74HfsHr(ReprMixin, Base74Hr, Smf74hfs):
    """The Smf74HfsHr class stores the hourly Smf74Hfs record in the smf74_hfs_hr table."""

    __tablename__ = "smf74_hfs_hr"
    smf_type: so.Mapped[Optional[str]] = so.mapped_column(sa.String(5), doc="the SMF type and subtype.")
    date: so.Mapped[Optional[dt.date]] = so.mapped_column(sa.Date, doc="the date of the record.")
    last_update_time: so.Mapped[Optional[dt.datetime]] = so.mapped_column(sa.DateTime,
                                                                          doc="last update time of this record.")
    ...
    smf74_pro_hr: so.Mapped['Smf74ProHr'] = so.relationship(back_populates='smf74_hfs_hr', viewonly=True)
    smf74_gbuf_hrs: so.Mapped[List['Smf74GbufHr']] = so.relationship(back_populates='smf74_hfs_hr', viewonly=True)
    smf74_fsys_hrs: so.Mapped[List['Smf74FsysHr']] = so.relationship(back_populates='smf74_hfs_hr', viewonly=True)


class Smf74LssHr(ReprMixin, Base74Hr, Smf74lss):
    """The Smf74LssHr class stores the hourly Smf74Lss record in the smf74_lss_hr table."""

    __tablename__ = "smf74_lss_hr"
    ...
    __table_args__ = (
        sa.PrimaryKeyConstraint(datetime, r748cser, r748laid),
        sa.ForeignKeyConstraint(
            ['datetime', 'r748cser'],
            ['smf74_cntl_hr.datetime', 'smf74_cntl_hr.r748cser']),
    )

    smf74_cntl_hr: so.Mapped['Smf74CntlHr'] = so.relationship(back_populates='smf74_lss_hrs', viewonly=True)


class Smf74MbrHr(ReprMixin, Base74Hr, Smf74mbr):
    """The Smf74MbrHr class stores the hourly Smf74Mbr record in the smf74_mbr_hr table."""

    __tablename__ = "smf74_mbr_hr"
    ...
    __table_args__ = (
        sa.PrimaryKeyConstraint(csc, smf74sid, datetime, r742mgrp, r742mmem, r742msys),
        sa.ForeignKeyConstraint(
            ['csc', 'smf74sid', 'datetime'],
            ['smf74_xctl_hr.csc', 'smf74_xctl_hr.smf74sid', 'smf74_xctl_hr.datetime']),
    )

    smf74_xctl_hr: so.Mapped['Smf74XctlHr'] = so.relationship(back_populates='smf74_mbr_hrs', viewonly=True)


class Smf74MscmHr(ReprMixin, Base74Hr, Smf74mscm):
    """The Smf74MscmHr class stores the hourly Smf74Mscm record in the smf74_mscm_hr table."""

    __tablename__ = "smf74_mscm_hr"
    csc: so.Mapped[Optional[str]] = so.mapped_column(sa.String(8),
                                                     doc="the identifier or serial number of Central Processor Complexes (CPC).")
    date: so.Mapped[Optional[dt.date]] = so.mapped_column(sa.Date, doc="the date of the record.")
    last_update_time: so.Mapped[Optional[dt.datetime]] = so.mapped_column(sa.DateTime,
    ...
    smf74_sreq_hr: so.Mapped['Smf74SreqHr'] = so.relationship(back_populates='smf74_mscm_hrs', viewonly=True)
    smf74_str_hr: so.Mapped['Smf74StrHr'] = so.relationship(back_populates='smf74_mscm_hrs', viewonly=True)
    smf74_cf_hr: so.Mapped['Smf74CfHr'] = so.relationship(back_populates='smf74_mscm_hrs', viewonly=True)


class Smf74OmvsHr(ReprMixin, Base74Hr, Smf74omvs):
    """The Smf74OmvsHr class stores the hourly Smf74Omvs record in the smf74_omvs_hr table."""

    __tablename__ = "smf74_omvs_hr"
    smf_type: so.Mapped[Optional[str]] = so.mapped_column(sa.String(5), doc="the SMF type and subtype.")
    date: so.Mapped[Optional[dt.date]] = so.mapped_column(sa.Date, doc="the date of the record.")
    last_update_time: so.Mapped[Optional[dt.datetime]] = so.mapped_column(sa.DateTime,
                                                                          doc="last update time of this record.")
    ...
    smf74_pro_hr: so.Mapped['Smf74ProHr'] = so.relationship(back_populates='smf74_omvs_hr', viewonly=True)


class Smf74PathHr(ReprMixin, Base74Hr, Smf74path):
    """The Smf74PathHr class stores the hourly Smf74Path record in the smf74_path_hr table."""

    __tablename__ = "smf74_path_hr"
    ...
    __table_args__ = (
        sa.PrimaryKeyConstraint(csc, smf74sid, datetime, r742pnme, r742pdir, r742pona, r742ptyp, r742pstr, r742ptcn),
        sa.ForeignKeyConstraint(
            ['csc', 'smf74sid', 'datetime'],
            ['smf74_xctl_hr.csc', 'smf74_xctl_hr.smf74sid', 'smf74_xctl_hr.datetime']),
    )

    smf74_xctl_hr: so.Mapped['Smf74XctlHr'] = so.relationship(back_populates='smf74_path_hrs', viewonly=True)


class Smf74PcieHr(ReprMixin, Base74Hr, Smf74pcie, Smf74dma0, Smf74dma1, Smf74dma2, Smf74dma3, Smf74dma4, Smf74hwa,
                  Smf74hwa1):
    """The Smf74PcieHr class stores the hourly Smf74Pcie record in the smf74_pcie_hr table."""

    __tablename__ = "smf74_pcie_hr"
    ...
    __table_args__ = (
        sa.PrimaryKeyConstraint(datetime, r749pfid),
    )

    smf74_pro_hrs: so.Mapped[List['Smf74ProHr']] = so.relationship(back_populates='smf74_pcie_hr', viewonly=True,
                                                                   foreign_keys='Smf74ProHr.datetime, Smf74ProHr.smf_type',
                                                                   primaryjoin='and_(Smf74PcieHr.datetime==Smf74ProHr.datetime, Smf74PcieHr.smf_type==Smf74ProHr.smf_type)', )
    smf74_srtd_hrs: so.Mapped[List['Smf74SrtdHr']] = so.relationship(back_populates='smf74_pcie_hr', viewonly=True)


class Smf74PortHr(ReprMixin, Base74Hr, Smf74port, Smf74pstat):
    """The Smf74PortHr class stores the hourly Smf74Port record in the smf74_port_hr table."""

    __tablename__ = "smf74_port_hr"
    smf74sid: so.Mapped[Optional[str]] = so.mapped_column(sa.String(4),
                                                          doc="System identification (from the SMFPRMxx SID parameter).")
    date: so.Mapped[Optional[dt.date]] = so.mapped_column(sa.Date, doc="the date of the record.")
    last_update_time: so.Mapped[Optional[dt.datetime]] = so.mapped_column(sa.DateTime,
    ...
    __table_args__ = (
        sa.PrimaryKeyConstraint(datetime, r747sdev, r747padr, r747pnum),
        sa.ForeignKeyConstraint(
            ['datetime', 'r747sdev'],
            ['smf74_switch_hr.datetime', 'smf74_switch_hr.r747sdev']),
    )

    smf74_switch_hr: so.Mapped['Smf74SwitchHr'] = so.relationship(back_populates='smf74_port_hrs', viewonly=True)
    smf74_connector_hrs: so.Mapped[List['Smf74ConnectorHr']] = so.relationship(back_populates='smf74_port_hr',
                                                                               viewonly=True)


class Smf74ProcHr(ReprMixin, Base74Hr, Smf74proc):
    """The Smf74ProcHr class stores the hourly Smf74Proc record in the smf74_proc_hr table."""

    __tablename__ = "smf74_proc_hr"
    ...
    __table_args__ = (
        sa.PrimaryKeyConstraint(smf74xnm, datetime, r744fnam, r744pnum, smf74sid),
        sa.ForeignKeyConstraint(
            ['smf74xnm', 'datetime', 'r744fnam'],
            ['smf74_cf_hr.smf74xnm', 'smf74_cf_hr.datetime', 'smf74_cf_hr.r744fnam']),
    )

    smf74_cf_hr: so.Mapped['Smf74CfHr'] = so.relationship(back_populates='smf74_proc_hrs', viewonly=True)


class Smf74ScmHr(ReprMixin, Base74Hr, Smf74scm):
    """The Smf74ScmHr class stores the hourly Smf74Scm record in the smf74_scm_hr table."""

    __tablename__ = "smf74_scm_hr"
    ...
    __table_args__ = (
        sa.PrimaryKeyConstraint(csc, smf74sid, datetime, r7410crid, r7410cpid),
        sa.ForeignKeyConstraint(
            ['datetime', 'smf_type', 'csc', 'smf74sid'],
            ['smf74_pro_hr.datetime', 'smf74_pro_hr.smf_type', 'smf74_pro_hr.csc', 'smf74_pro_hr.smf74sid']),
    )

    smf74_pro_hrs: so.Mapped[List['Smf74ProHr']] = so.relationship(back_populates='smf74_scm_hr', viewonly=True)


class Smf74SiolHr(ReprMixin, Base74Hr, Smf74siol):
    """The Smf74SiolHr class stores the hourly Smf74Siol record in the smf74_siol_hr table."""

    __tablename__ = "smf74_siol_hr"
    ...
    __table_args__ = (
        sa.PrimaryKeyConstraint(datetime, r748cser, r748siid),
        sa.ForeignKeyConstraint(
            ['datetime', 'r748cser'],
            ['smf74_cntl_hr.datetime', 'smf74_cntl_hr.r748cser']),
    )

    smf74_cntl_hr: so.Mapped['Smf74CntlHr'] = so.relationship(back_populates='smf74_siol_hrs', viewonly=True)


class Smf74SreqHr(ReprMixin, Base74Hr, Smf74sreq, Smf74cach):
    """The Smf74SreqHr class stores the hourly Smf74Sreq record in the smf74_sreq_hr table."""

    __tablename__ = "smf74_sreq_hr"
    r744snam: so.Mapped[str] = so.mapped_column(sa.String(16),
                                                doc="Name of connected structure in this coupling facility.")
    ...
    __table_args__ = (
        ...
        sa.ForeignKeyConstraint(
            ...
            ['smf74_str_hr.smf74xnm', 'smf74_str_hr.datetime', 'smf74_str_hr.r744fnam', 'smf74_str_hr.r744qstr']),
        sa.ForeignKeyConstraint(
            ['smf74xnm', 'datetime', 'r744fnam', 'smf74sid'],
            ['smf74_lcf_hr.smf74xnm', 'smf74_lcf_hr.datetime', 'smf74_lcf_hr.r744fnam', 'smf74_lcf_hr.smf74sid']),
    )

    smf74_lcf_hr: so.Mapped['Smf74LcfHr'] = so.relationship(back_populates='smf74_sreq_hrs', viewonly=True)
    smf74_str_hr: so.Mapped['Smf74StrHr'] = so.relationship(back_populates='smf74_sreq_hrs', viewonly=True)
    smf74_cach_hrs: so.Mapped[List['Smf74CachHr']] = so.relationship(back_populates='smf74_sreq_hr', viewonly=True)
    smf74_mscm_hrs: so.Mapped[List['Smf74MscmHr']] = so.relationship(back_populates='smf74_sreq_hr', viewonly=True)
    smf74_adup_hrs: so.Mapped[List['Smf74AdupHr']] = so.relationship(back_populates='smf74_sreq_hr', viewonly=True)


class Smf74SrtdHr(ReprMixin, Base74Hr, Smf74srtd):
    """The Smf74SrtdHr class stores the hourly Smf74Srtd record in the smf74_srtd_hr table."""

    __tablename__ = "smf74_srtd_hr"
    ...
    __table_args__ = (
        sa.PrimaryKeyConstraint(datetime, r749pfid, response_time_for_sync_io_write, srtd_idx, r749rtrv),
        sa.ForeignKeyConstraint(
            ['datetime', 'r749pfid'],
            ['smf74_pcie_hr.datetime', 'smf74_pcie_hr.r749pfid']),
    )

    smf74_pcie_hr: so.Mapped['Smf74PcieHr'] = so.relationship(back_populates='smf74_srtd_hrs', viewonly=True)


class Smf74StrHr(ReprMixin, Base74Hr, Smf74str, Smf74sreq, Smf74cach, Smf74mscm, Smf74adup):
    """The Smf74StrHr class stores the hourly Smf74Str record in the smf74_str_hr table."""

    __tablename__ = "smf74_str_hr"
    ...
    __table_args__ = (
        sa.PrimaryKeyConstraint(smf74xnm, datetime, r744fnam, r744qstr),
        sa.ForeignKeyConstraint(
            ['smf74xnm', 'datetime', 'r744fnam'],
            ['smf74_cf_hr.smf74xnm', 'smf74_cf_hr.datetime', 'smf74_cf_hr.r744fnam']),
    )

    smf74_cf_hr: so.Mapped['Smf74CfHr'] = so.relationship(back_populates='smf74_str_hrs', viewonly=True)
    smf74_sreq_hrs: so.Mapped[List['Smf74SreqHr']] = so.relationship(back_populates='smf74_str_hr', viewonly=True)
    smf74_mscm_hrs: so.Mapped[List['Smf74MscmHr']] = so.relationship(back_populates='smf74_str_hr', viewonly=True)
    smf74_adup_hrs: so.Mapped[List['Smf74AdupHr']] = so.relationship(back_populates='smf74_str_hr', viewonly=True)


class Smf74SwitchHr(ReprMixin, Base74Hr, Smf74switch, Smf74pstat):
    """The Smf74SwitchHr class stores the hourly Smf74Switch record in the smf74_switch_hr table."""

    __tablename__ = "smf74_switch_hr"
    r747sdev: so.Mapped[str] = so.mapped_column(sa.String(4), doc="Switch device number.")
    last_update_time: so.Mapped[Optional[dt.datetime]] = so.mapped_column(sa.DateTime,
                                                                          doc="last update time of this record.")
    date: so.Mapped[Optional[dt.date]] = so.mapped_column(sa.Date, doc="the date of the record.")
    ...
    __table_args__ = (
        sa.PrimaryKeyConstraint(datetime, r747sdev),
        sa.ForeignKeyConstraint(
            ['datetime'],
            ['smf74_fcd_hr.datetime']),
    )

    smf74_fcd_hr: so.Mapped['Smf74FcdHr'] = so.relationship(back_populates='smf74_switch_hrs', viewonly=True)
    smf74_port_hrs: so.Mapped[List['Smf74PortHr']] = so.relationship(back_populates='smf74_switch_hr', viewonly=True)


class Smf74SysHr(ReprMixin, Base74Hr, Smf74sys):
    """The Smf74SysHr class stores the hourly Smf74Sys record in the smf74_sys_hr table."""

    __tablename__ = "smf74_sys_hr"
    ...
    __table_args__ = (
        sa.PrimaryKeyConstraint(csc, smf74sid, datetime, r742snme, r742stcn, r742sdir),
        sa.ForeignKeyConstraint(
            ['csc', 'smf74sid', 'datetime'],
            ['smf74_xctl_hr.csc', 'smf74_xctl_hr.smf74sid', 'smf74_xctl_hr.datetime']),
    )

    smf74_xctl_hr: so.Mapped['Smf74XctlHr'] = so.relationship(back_populates='smf74_sys_hrs', viewonly=True)


class Smf74XctlHr(ReprMixin, Base74Hr):
    """The Smf74XctlHr class stores the hourly Smf74Xctl record in the smf74_xctl_hr table."""

    __tablename__ = "smf74_xctl_hr"
    smf_type: so.Mapped[Optional[str]] = so.mapped_column(sa.String(5), doc="the SMF type and subtype.")
    date: so.Mapped[Optional[dt.date]] = so.mapped_column(sa.Date, doc="the date of the record.")
    last_update_time: so.Mapped[Optional[dt.datetime]] = so.mapped_column(sa.DateTime,
                                                                          doc="last update time of this record.")
    ...
    smf74_pro_hr: so.Mapped['Smf74ProHr'] = so.relationship(back_populates='smf74_xctl_hr', viewonly=True)
    smf74_sys_hrs: so.Mapped[List['Smf74SysHr']] = so.relationship(back_populates='smf74_xctl_hr', viewonly=True)
    smf74_path_hrs: so.Mapped[List['Smf74PathHr']] = so.relationship(back_populates='smf74_xctl_hr', viewonly=True)
    smf74_mbr_hrs: so.Mapped[List['Smf74MbrHr']] = so.relationship(back_populates='smf74_xctl_hr', viewonly=True)


class Smf74RaidHr(ReprMixin, Base74Hr, Smf74raid, Smf741dev):
    """The Smf74RaidHr class stores the hourly Smf74Raid record in the smf74_raid_hr table."""

    __tablename__ = "smf74_raid_hr"
    ...
    __table_args__ = (
        ...
        sa.ForeignKeyConstraint(
            ['datetime', 'r7451rid'],
            ['smf74_rrank_hr.datetime', 'smf74_rrank_hr.r7451rid']),
        sa.ForeignKeyConstraint(
            ['datetime', 'r745ssid', 'r7451dvn'],
            ['smf74_cdev_hr.datetime', 'smf74_cdev_hr.r745dsid', 'smf74_cdev_hr.r745devn']),
    )

    smf74_cdev_hr: so.Mapped['Smf74CdevHr'] = so.relationship(back_populates='smf74_raid_hr', viewonly=True)
    smf74_rrank_hr: so.Mapped['Smf74RrankHr'] = so.relationship(back_populates='smf74_raid_hrs', viewonly=True)


class Smf74RrankHr(ReprMixin, Base74Hr, Smf74raid, Smf741dev):
    """The Smf74RrankHr class stores the hourly Smf74Rrank record in the smf74_rrank_hr table."""

    __tablename__ = "smf74_rrank_hr"
    ...
    __table_args__ = (
        sa.PrimaryKeyConstraint(datetime, r7451rid),
        sa.ForeignKeyConstraint(
            ['datetime', 'r745ssid'],
            ['smf74_cachsys_hr.datetime', 'smf74_cachsys_hr.r745ssid']),
    )

    smf74_cachsys_hr: so.Mapped['Smf74CachsysHr'] = so.relationship(back_populates='smf74_rrank_hrs', viewonly=True)
    smf74_raid_hrs: so.Mapped[List['Smf74RaidHr']] = so.relationship(back_populates='smf74_rrank_hr', viewonly=True)


class Smf74CfHr(ReprMixin, Base74Hr, Smf74cf, Smf74lcf, Smf74gsrg):
    """The Smf74CfHr class stores the hourly Smf74Cf record in the smf74_cf_hr table."""

    __tablename__ = "smf74_cf_hr"
    ...
    __table_args__ = (
        sa.PrimaryKeyConstraint(smf74xnm, datetime, r744fnam),
    )

    smf74_lcf_hrs: so.Mapped[List['Smf74LcfHr']] = so.relationship(back_populates='smf74_cf_hr', viewonly=True)
    smf74_str_hrs: so.Mapped[List['Smf74StrHr']] = so.relationship(back_populates='smf74_cf_hr', viewonly=True)
    smf74_cfrf_hrs: so.Mapped[List['Smf74CfrfHr']] = so.relationship(back_populates='smf74_cf_hr', viewonly=True)
    smf74_proc_hrs: so.Mapped[List['Smf74ProcHr']] = so.relationship(back_populates='smf74_cf_hr', viewonly=True)
    smf74_subchpa_hrs: so.Mapped[List['Smf74SubchpaHr']] = so.relationship(back_populates='smf74_cf_hr', viewonly=True)
    smf74_mscm_hrs: so.Mapped[List['Smf74MscmHr']] = so.relationship(back_populates='smf74_cf_hr', viewonly=True)
